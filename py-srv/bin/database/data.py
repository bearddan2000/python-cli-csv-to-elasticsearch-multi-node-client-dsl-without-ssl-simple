...
def main():
    ...
    with open('{}.csv'.format(INDEX_NAME), "r") as fi:
        ...
        actions = []
        for row in reader:
            split_field = row[COLUMN_NAME['col_3']['name']].split('::')
            csv_doc = {
                ...
                COLUMN_NAME['col_3']['name']: split_field,
                COLUMN_NAME['col_4']['name']: float(row[COLUMN_NAME['col_4']['name']]),
                COLUMN_NAME['col_5']['name']: int(row[COLUMN_NAME['col_5']['name']])
            }
